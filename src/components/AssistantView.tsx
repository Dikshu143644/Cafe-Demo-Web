...
import { 
  ...
  Mic, MicOff, Volume2, VolumeX, Info, AlertCircle, ShoppingBag, CheckCircle
} from 'lucide-react';
...
export default function AssistantView({ 
  ...
  const processAutomatedAction = async (action: any) => {
    ...
    if (action.type === 'add_to_cart') {
      ...
      if (finalItem) {
        ...
        setMessages((prev) => {
          const newMsgs = [...prev, { sender: 'assistant', text: autoMsgText }];
          if (speechEnabled) {
            setTimeout(() => {
  ...
  const handleSend = async (textToSend: string) => {
    ...
    try {
      ...
      setMessages((prev) => {
        const newMsgs = [...prev, { sender: 'assistant', text: result.response }];
    ...
    <div id="ai-voice-assistant-page" className="bg-transparent min-h-[calc(100vh-130px)] pt-16 sm:pt-24 pb-8 sm:pb-16 font-sans text-cafe-smoky relative z-10 flex flex-col justify-start">
      <div className="max-w-5xl mx-auto px-2 sm:px-6 w-full flex flex-col justify-start">
        ...
        <div className="text-center space-y-1 mb-4 sm:mb-8 shrink-0">
          ...
          <h1 className="font-serif text-xl sm:text-4xl lg:text-5xl font-extrabold uppercase text-cafe-charcoal tracking-tight mt-0.5 sm:mt-2 font-semibold">
          ...
          <p className="hidden md:block text-xs sm:text-sm text-cafe-charcoal/80 max-w-xl mx-auto leading-relaxed">
            A dual-engine Conversational AI that lets you talk, auto-fill reservations, and place orders directly to our kitchen using natural speech.
          </p>
        </div>
            ...
            <GlassCard theme="light" className="h-full w-full flex flex-col justify-between p-0 overflow-hidden relative shadow-2xl border-2 border-[#A88665]/40" hoverEffect={false}>
              ...
              <div className="px-5 py-3.5 border-b border-[#A88665]/20 bg-stone-50/90 flex items-center justify-between shrink-0">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <span className="text-[11px] font-extrabold uppercase tracking-wider text-[#1C1814] block">Aura Voice Engine</span>
                    {isListening ? (
                      <div className="flex items-center space-x-2 mt-0.5">
                        <span className="text-[8px] uppercase tracking-widest text-red-600 font-extrabold animate-pulse font-mono block">● Aura listening...</span>
                      ...
                      <span className="text-[8px] uppercase tracking-widest text-[#A47E54] font-extrabold block font-mono mt-0.5 animate-pulse">● Voice ready / manually read</span>
                    )}
                  </div>
                </div>
              ...
              <div 
                ...
                className="flex-1 overflow-y-auto px-5 py-4 space-y-4 bg-[#FDFBF7]/40 touch-pan-y overscroll-contain"
                  ...
                  >
                    <div className="flex items-start space-x-2 max-w-[85%]">
                      {msg.sender === 'assistant' && (
                        <div className="w-6 h-6 rounded-full bg-stone-200/50 border border-stone-300 flex items-center justify-center shrink-0 mt-0.5 text-[#1C1814]">
                          🤖
                        </div>
                      )}
                  ...
                  <div className="flex justify-start animate-fadeIn">
                    <div className="flex items-center space-x-3 px-4 py-2 bg-amber-50 border border-[#A88665]/20 rounded-2xl shadow-sm">
                      ...
                      >
                        ...
                        <span>STOP COGNITIVE AUDIO</span>
                      </button>
                    </div>
                  </div>
                )}
                ...
                {isTyping && (
                  <div className="flex justify-start animate-pulse">
                    <div className="bg-white border border-[#A88665]/20 text-cafe-smoky rounded-2xl rounded-tl-none px-3.5 py-2.5 text-xs flex items-center space-x-1 shadow-sm">
                      <span className="w-1.5 h-1.5 rounded-full bg-cafe-gold animate-bounce"></span>
                      <span className="w-1.5 h-1.5 rounded-full bg-cafe-gold animate-bounce [animation-delay:0.2s]"></span>
                      <span className="w-1.5 h-1.5 rounded-full bg-cafe-gold animate-bounce [animation-delay:0.4s]"></span>
                    </div>
                  </div>
                )}
