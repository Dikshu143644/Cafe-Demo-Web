...
export default function GlassCard({
  ...
  return (
    <div
      id={id}
      className={`${baseStyle} ${hoverStyle} rounded-2xl p-6 overflow-hidden relative ${className}`}
    >
      ...
      <div className={`relative z-10 ${className.includes('h-') ? 'h-full flex flex-col' : ''}`}>{children}</div>
    </div>
  );
}
