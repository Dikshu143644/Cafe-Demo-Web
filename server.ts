...
async function seedDatabaseIfNeeded() {
  try {
    ...
    if (existingCount.length === 0) {
      console.log('Database empty on startup! Undergoing auto-seeding Café Vista premium menu items...');
      const seedItems = [
        {
          id: 'm1',
...
async function startServer() {
  ...
  app.post('/api/auth/signup', apiRateLimiter({ windowMs: 60000, max: 5, message: 'Limit reached. Please wait a minute before attempting registration.' }), async (req, res) => {
    ...
    try {
      ...
      if (existing) {
        return res.status(409).json({ success: false, message: 'An account with this email address already matches database records' });
      }

      // Hash passwords using bcrypt securely
      const salt = bcrypt.genSaltSync(12);
      ...
      // Insert User to Cloud SQL database directly
    ...
    "tablePreference": "window" | "garden" | "lounge" | null
      ...
      if (!responseText) {
        if (query.includes('recommend') || query.includes('suggest') || query.includes('menu') || query.includes('popular')) {
          ...
          responseText = `Hello! I am your **Café Vista Digital Concierge & Voice Guide** ☕ I can help you with menu items, scheduling booking, or tracking order history! Let me know what you'd like.`;
        }
      }
